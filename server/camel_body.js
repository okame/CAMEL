...
(function() {
		...
		//TODO:change scope(kondo)
		//manage packman ID
		packmanidArray = new Array();

		//manage to finish execution
		checkFinishArray = new Array(packSrv.env.PACK_NUM);
			...
			this.operations = {
					...
					if( packmanidArray[con.id] != null || id <= env.PACK_NUM || checkFinishArray[id] == false ) {
						...
						if(flag){
							...
							*/
							...
							that.sv.broadcast( that.createMsg('prestart', env) );
						}
					}else{
				...
				ack : function(con) {
					...
					if(flag){
						...
						that.sv.broadcast( that.createMsg('start', '') );
